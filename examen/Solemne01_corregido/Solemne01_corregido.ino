...
void setup() {
  ...
  // configura el pin del LED de estado como salida
...
void loop() {
  ...
  if (estadoBoton != ultimoEstadoBoton) {
    if (estadoBoton == HIGH) {
      ...
      estadoBoton = digitalRead(pinBoton); // verificar el estado de nuevo
      if (estadoBoton == HIGH) { // asegurarse de que sigue presionado
  ...
  // actualizar el estado del LED de estado
  if (juegoEncendido) {
